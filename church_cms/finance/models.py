...
class Fund(models.Model):
    ...
    code = models.CharField(
        ...
        editable=False,
        blank=True,
        help_text='Auto-generated unique fund code',
    ...
    def save(self, *args, **kwargs):
        if not self.code:
            prefix = self.name[:3].upper() if self.name else 'FND'

            while True:
                generated_code = f"{prefix}-{uuid.uuid4().hex[:5].upper()}"

                if not Fund.objects.filter(code=generated_code).exists():
